...
impl<'a> BorrowedBuf<'a> {
    ...
    pub fn unfilled<'b>(&'b mut self) -> BorrowedCursor<'b> {
        BorrowedCursor {
            start: self.filled,
            buf:   unsafe { mem::transmute::<&'b mut BorrowedBuf<'a>, &'b mut BorrowedBuf<'b>>(self) },
        }
    }
...
impl<'a> BorrowedCursor<'a> {
    ...
    pub fn reborrow<'b>(&'b mut self) -> BorrowedCursor<'b> {
        BorrowedCursor {
            buf:   unsafe { mem::transmute::<&'b mut BorrowedBuf<'a>, &'b mut BorrowedBuf<'b>>(self.buf) },
            start: self.start,
        }
    }
    #[inline]
...
impl<'a> From<&'a mut [u8]> for BorrowedBuf<'a> {
    #[inline]
    fn from(slice: &'a mut [u8]) -> BorrowedBuf<'a> {
        let n = slice.len();
        BorrowedBuf {
            buf:    stx::take(unsafe { (slice as *mut [u8]).as_uninit_slice_mut() }),
            init:   n,
            filled: 0,
        }
    }
...
impl<'a> From<&'a mut [MaybeUninit<u8>]> for BorrowedBuf<'a> {
    #[inline]
    fn from(buf: &'a mut [MaybeUninit<u8>]) -> BorrowedBuf<'a> {
