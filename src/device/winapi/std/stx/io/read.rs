...
use crate::util::stx::io::{self, BorrowedBuf, BorrowedCursor, Error, ErrorKind, Seek, SeekFrom, Write};
...
impl Buffer {
    ...
    fn with_capacity(capacity: usize) -> Buffer {
        ...
        Buffer { buf, pos: 0, init: 0, filled: 0 }
    }
...
impl<T: Read> BufReader<T> {
    ...
    pub fn new(inner: T) -> BufReader<T> {
        BufReader {
            inner,
            buf: Buffer::with_capacity(io::BUF_SIZE),
        }
    }
    #[inline]
...
fn memchr_aligned(x: u8, text: &[u8]) -> Option<usize> {
    let n = text.len();
    let p = text.as_ptr();
    let mut o = p.align_offset(io::PTR_SIZE);
    if o > 0 {
        o = cmp::min(o, n);
    ...
    while o <= n - 2 * io::PTR_SIZE {
        unsafe {
            if io::zero_byte((*(p.add(o) as *const usize)) ^ r) || io::zero_byte((*(p.add(o + io::PTR_SIZE) as *const usize)) ^ r) {
                break;
            }
        }
        o += io::PTR_SIZE * 2;
