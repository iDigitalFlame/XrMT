...
#![no_implicit_prelude]
#![cfg(all(target_family = "windows", not(feature = "std"), target_arch = "x86_64"))]
...
pub(super) unsafe fn compare(a: *const u8, b: *const u8, n: usize) -> i32 {
    ...
    let l = |a: *const u128, b, n| {
        _cmp(a, b, n, |a: *const u64, b, n| {
            _cmp(a, b, n, |a: *const u32, b, n| {
                _cmp(a, b, n, |a: *const u16, b, n| _cmp(a, b, n, g))
            })
        })
    };
    l(a.cast(), b.cast(), n)
...
unsafe fn _cmp<T: Clone + Copy + Eq, U: Clone + Copy + Eq, F: FnOnce(*const U, *const U, usize) -> i32>(mut a: *const T, mut b: *const T, n: usize, f: F) -> i32 {
    let end = a.add(unchecked_div(n, size_of::<T>()));
    while a != end {
        if a.read_unaligned() != b.read_unaligned() {
            return f(a.cast(), b.cast(), size_of::<T>());
        }
        a = a.add(1);
        b = b.add(1);
    }
    f(a.cast(), b.cast(), unchecked_rem(n, size_of::<T>()))
}
