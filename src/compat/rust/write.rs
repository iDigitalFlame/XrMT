...
use core::{cmp, error, ptr};

use crate::ignore_error;
use crate::io::{self, Error, ErrorKind, Seek, SeekFrom};
...
impl<T: Write> BufWriter<T> {
    ...
    pub fn new(inner: T) -> BufWriter<T> {
        BufWriter {
            inner,
            buf: Vec::with_capacity(super::BUF_SIZE),
            panicked: false,
        }
    }
    #[inline]
...
impl Write for &mut [u8] {
    ...
    fn write(&mut self, data: &[u8]) -> io::Result<usize> {
        ...
        let (a, b) = replace(self, &mut []).split_at_mut(n);
...
fn memrchr(x: u8, text: &[u8]) -> Option<usize> {
    ...
    while o > n {
        unsafe {
            if super::zero_byte((*(p.add(o - 2 * super::PTR_SIZE) as *const usize)) ^ r) || super::zero_byte((*(p.add(o - super::PTR_SIZE) as *const usize)) ^ r) {
                break;
            }
        }
        o -= 2 * super::PTR_SIZE;
