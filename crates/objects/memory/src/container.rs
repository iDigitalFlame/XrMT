...
use core::ops::{BitXorAssign, Drop};
use core::option::Option::{self, None, Some};
use core::ptr::{read, replace, slice_from_raw_parts_mut, swap, write, NonNull};
...
const INITIAL_MIN: usize = 512usize;
const INITIAL_MULTIPLY: usize = 3usize;

pub struct Silo<'a> {
    v:  NonNull<Entry>,
    _p: PhantomData<&'a Container>,
}
pub struct AllocMutIter<'a> {
    pos:  usize,
    root: &'a Entry,
}
pub struct Container(Box<Entry>);
pub struct LeafIter<'a>(&'a Entry);
pub struct AllocIter<'a>(AllocMutIter<'a>);
...
struct Entry(UnsafeCell<NonNull<Block>>);
...
impl Heap {
    ...
    fn read<T>(&self, pos: usize) -> T {
        unsafe { read((self.0.as_ptr() as *const u8).add(pos) as *const T) }
    }
    #[inline]
    fn write<T>(&self, pos: usize, v: T) {
        unsafe { write(self.0.as_ptr().add(pos) as *mut T, v) }
    }
    #[inline]
    ...
    #[inline]
    fn bytes<'a>(&'a self, pos: usize, size: usize) -> &'a mut [u8] {
        // Take off the header.
        unsafe { from_raw_parts_mut(self.0.as_ptr().add(pos + Header::SIZE), size) }
    }
    #[inline]
    fn slice(&self, pos: usize, size: usize, align: usize) -> NonNull<[u8]> {
        unsafe {
            NonNull::new_unchecked(slice_from_raw_parts_mut(
                // Take off the header.
                self.0.as_ptr().add(pos + align + Header::SIZE),
                size,
            ))
        }
    }
...
impl Entry {
    ...
    const fn empty() -> Entry {
        Entry(UnsafeCell::new(NonNull::dangling()))
    }
    ...
    /// Walk down the tree, cutting off any sections that contain empty links
    /// but rejoin ones that may have data in them.
    fn trim(&self) {
    ...
    #[inline]
    fn unlink(&self) {
        unsafe { replace(self.0.get(), NonNull::dangling()) };
    }
    #[inline]
    fn is_empty(&self) -> bool {
        unsafe { &*self.0.get() }.as_ptr() as usize == Header::SIZE
    }
    #[inline]
    fn ptr(&self) -> &mut Block {
        unsafe { (&mut *self.0.get()).as_mut() }
    }
    #[inline]
    fn replace(&self, new: &Entry) {
        unsafe { swap(self.0.get(), new.0.get()) };
        let _ = new.unlink(); // Drop replaced Entry
    }
    #[inline]
    fn as_slice(&self) -> NonNull<[u8]> {
        unsafe {
            NonNull::new_unchecked(slice_from_raw_parts_mut(
                self.ptr().mem.0.as_ptr() as *mut u8,
                self.ptr().size,
            ))
        }
    }
    ...
    fn deallocate(&self, ptr: NonNull<u8>, layout: Layout) {
        ...
        self.ptr().deallocate(ptr.as_ptr() as usize, layout);
    }
    #[inline]
    fn init(&self, layout: Layout) -> Result<(), AllocError> {
        let _ = unsafe { replace(self.0.get(), Block::new(layout)?) };
        Ok(())
    }
    #[inline]
...
impl Block {
    ...
    fn new(layout: Layout) -> Result<NonNull<Block>, AllocError> {
        ...
        Ok(Block::write(Block {
            mem:  Heap::allocate(unsafe { Layout::from_size_align_unchecked(n, Header::SIZE).pad_to_align() })?,
            next: Entry::empty(),
            size: n,
        }))
    }
    ...
    fn free(&self) {
        self.mem
            .free(unsafe { Layout::from_size_align_unchecked(self.size, Header::SIZE) }.pad_to_align());
    }
    #[inline]
    fn is_empty(&self) -> bool {
        if self.size == 0 {
        ...
        let h = self.mem.read::<Header>(Header::SIZE);
        if h.0 == 0 {
    ...
    fn write(b: Block) -> NonNull<Block> {
        let m = b.mem.0.as_ptr();
        let p = unsafe { NonNull::new_unchecked(m as *mut Block) };
        unsafe {
            write(m.add(Block::SIZE) as *mut Header, Header(0));
            write(m as *mut Block, b);
        }
        p
    }
    #[inline]
    fn iter<'a>(&'a self) -> BlockIter<'a> {
        BlockIter { ptr: self, pos: 0usize }
    }
    fn walk(&self, pos: usize) -> Option<Address> {
        let p = if pos > 0 { pos } else { pos + Block::SIZE };
        if p > self.size {
            return None;
        }
    ...
    fn deallocate(&self, addr: usize, layout: Layout) -> bool {
        match self.contains(addr, layout) {
            Some(a) => {
                ...
                // empty.
                let h = self.mem.read::<Header>(a.start);
                if h.is_zero() {
            ...
            None if !self.next.is_empty() => return self.next.ptr().deallocate(addr, layout),
            _ => {
    ...
    fn next(&self, layout: Layout) -> Option<(usize, Header, usize)> {
        // All addresses returned INCLUDE THE HEADER SIZE ptr.
        // This shouldn't fail, but check it anyway.
        let n = layout.size().checked_add(layout.align() + Header::SIZE)?;
        if n > self.size {
            // Reject any blocks that are too small.
            return None;
        }
        // Last seen empty Header and Seek position.
        let (mut k, mut p) = (0usize, Block::SIZE);
        loop {
            ...
            let a = if layout.align() > 1 {
                layout.align() - (self.mem.addr_at(p + Header::SIZE) & (layout.align() - 1))
            } else {
                0
            };
            let r = layout.size() + a;
            ...
            // fine.
            if (r + Header::SIZE) > self.size - p {
                return None;
            }
            // Read the Header at the current position.
            let h = self.mem.read::<Header>(p);
            if h.is_zero() {
                // Was the last Header free?
                if k > 0 {
            ...
            p = p.checked_add(h.size() + Header::SIZE)?;
        }
        // We ain't find shit.
        None
    }
    ...
    fn contains(&self, addr: usize, layout: Layout) -> Option<Address> {
        // Fast path: If we're empty, it's not possible.
        if self.is_empty() {
            return None;
        }
        let t = layout.size();
        ...
        let n = addr.checked_sub(self.mem.addr() + Block::SIZE + layout.align())?;
...
impl Header {
    ...
    const FLAG: usize = unsafe { 1usize.unchecked_shl(usize::BITS - 1) };
    ...
    #[inline]
    fn is_empty(&self) -> bool {
        // Check if zero or the last bit is unset
        self.0 == 0 || self.0 & Header::FLAG == 0
    }
}
impl Address {
    #[inline]
    fn matches(&self, mem: &Heap, addr: usize) -> bool {
        let r = self.start + mem.addr() + Header::SIZE;
...
impl Container {
    ...
    pub fn new() -> Container {
        // The root entry is in a Box to prevent it from getting moved when
        // it's on the stack and causing issues.
        Container(Box::new(Entry::empty()))
    }

    #[inline]
    pub const fn silo<'a>(&'a self) -> Silo<'a> {
        Silo {
            v:  unsafe { NonNull::new_unchecked((&*self.0 as *const Entry) as *mut Entry) },
            _p: PhantomData,
        }
    }
    ...
    pub fn wrap(&self, key: impl AsRef<[u8]>) {
        ...
        for i in self.iter_mut() {
            // 'iter_mut().enumerate()' should emit the bounds checks here.
            for (x, v) in i.iter_mut().enumerate() {
                // The compiler might not understand that 'x % n' will
                // always be in bounds of 'k'
                v.bitxor_assign(unsafe { *k.get_unchecked(x % n) });
            }
        }
    }
    #[inline]
    pub fn iter<'a>(&'a self) -> AllocIter<'a> {
        AllocIter(self.iter_mut())
    }
    #[inline]
    ...
    pub fn iter_mut<'a>(&'a self) -> AllocMutIter<'a> {
        AllocMutIter { root: &self.0, pos: 0usize }
    }
}
...
impl<'a> IntoIterator for &'a Container {
    type Item = &'a [u8];
    type IntoIter = AllocIter<'a>;

    #[inline]
    fn into_iter(self) -> AllocIter<'a> {
        self.iter()
    }
}

impl Copy for Silo<'_> {}
impl<'a> Clone for Silo<'a> {
    #[inline]
    fn clone(&self) -> Silo<'a> {
        Silo {
            v:  self.v.clone(),
            _p: PhantomData,
        }
...
impl Iterator for LeafIter<'_> {
    ...
    fn next(&mut self) -> Option<NonNull<[u8]>> {
        ...
        self.0 = &self.0.ptr().next;
        Some(r)
    }
}
impl FusedIterator for LeafIter<'_> {}
...
impl<'a> Iterator for AllocIter<'a> {
    ...
    fn next(&mut self) -> Option<&'a [u8]> {
        // It's the same thing but as mut, so transmuting it is fine.
        unsafe { transmute(self.0.next()) }
    }
}
impl FusedIterator for AllocIter<'_> {}
...
impl<'a> Iterator for AllocMutIter<'a> {
    ...
    fn next(&mut self) -> Option<&'a mut [u8]> {
        ...
        match self.root.ptr().walk(self.pos) {
            Some(a) => {
                self.pos = a.next;
...
impl FusedIterator for AllocMutIter<'_> {}
...
unsafe impl Allocator for Silo<'_> {
    #[inline]
    fn allocate(&self, layout: Layout) -> Result<NonNull<[u8]>, AllocError> {
        unsafe { self.v.as_ref() }.allocate(layout)
    }

    #[inline]
    unsafe fn deallocate(&self, ptr: NonNull<u8>, layout: Layout) {
        unsafe { self.v.as_ref() }.deallocate(ptr, layout)
    }
}
...
mod display {
    ...
    use crate::container::Silo;

    impl Debug for Silo<'_> {
        #[inline]
        fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
            f.debug_tuple("Silo").field(&self.v.as_ptr()).finish()
        }
    }
    impl Display for Silo<'_> {
        #[inline]
        fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
            f.write_str("Silo")
        }
