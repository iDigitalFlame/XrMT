...
use xrmt_data::{Readable, Reader, Writable, Writer};
...
#[repr(C)]
#[allow(dead_code)] // TODO(dij): Update ths
struct WindowInfo {
    size:     u32,
    window:   Rect,
    client:   Rect,
...
impl Bounds {
    #[inline]
    pub const fn new(x1: u32, y1: u32, x2: u32, y2: u32) -> Bounds {
        match 1 {
...
impl Readable for Window {
    fn read_stream(&mut self, r: &mut impl Reader) -> IoResult<()> {
        ...
        r.read_into_str_lossy(&mut self.name)?;
...
pub(crate) unsafe extern "stdcall" fn _enum_window(h: usize, out: *mut Vec<Window>) -> u32 {
    let b = some_or_return!(unsafe { out.as_mut() }, 0);
    let n = unsafe {
        winapi::syscall!(
...
mod display {
    ...
    use crate::structs::{Bounds, Point, Window};
