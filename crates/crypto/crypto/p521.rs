...
impl Num {
    ...
    const WORD_SIZE_BYTE: usize = Num::WORD_SIZE / 8;
...
impl Point {
    ...
    fn write(&self, buf: &mut [u8]) -> IoResult<()> {
        if buf.len() < 133 {
    ...
    fn write_secret(&self, buf: &mut [u8]) -> IoResult<()> {
        if buf.len() < 65 {
            return Err(ErrorKind::InvalidInput.into());
        }
        let x = self.x * !self.z;
...
impl TryFrom<&[u8]> for Point {
    ...
    #[inline]
    fn try_from(value: &[u8]) -> IoResult<Point> {
        Ok(Point {
...
impl TryFrom<&[u8]> for Element {
    ...
    fn try_from(value: &[u8]) -> IoResult<Element> {
        if value.len() != 66 {
            return Err(ErrorKind::InvalidInput.into());
        }
        // TODO(dij): Make this Time constant
...
#[inline]
pub fn generate_secret(public: &[u8], private: &[u8], secret: &mut [u8]) -> IoResult<()> {
    if secret.len() < 65 {
        return Err(ErrorKind::InvalidInput.into());
    }
...
#[inline]
pub fn generate_pair(src: &mut impl Read, public: &mut [u8], private: &mut [u8]) -> IoResult<()> {
    if private.len() != 66 {
        return Err(ErrorKind::InvalidInput.into());
    }
    loop {
