...
impl<A: Allocator> KeyPair<A> {
    ...
    #[inline]
    pub fn write(&self, w: &mut impl Write) -> IoResult<()> {
        write_full(w, &&self.inner[PUBLIC])
    }
    #[inline]
    pub fn read(&mut self, r: &mut impl Read) -> IoResult<()> {
        read_full(r, &mut self.inner[PUBLIC])
    }
    #[inline]
    pub fn write_all(&self, w: &mut impl Write) -> IoResult<()> {
        write_full(w, &self.inner[PUBLIC])?;
        write_full(w, &self.inner[PRIVATE])?;
        write_full(w, &self.inner[SHARED])
    }
    #[inline]
    pub fn read_all(&mut self, r: &mut impl Read) -> IoResult<()> {
        let (p, k, s) = self.split();
        read_full(r, p)?;
        read_full(r, k)?;
        read_full(r, s)
    ...
    #[inline]
    pub fn fill_public(&mut self, public: &PublicKey) -> IoResult<()> {
        let (p, k, s) = self.split();
        p521::generate_secret(p, k, s)?;
    ...
    #[inline]
    pub fn fill_private(&mut self, private: &PrivateKey) -> IoResult<()> {
        let (p, k, s) = self.split();
