...
impl Filter {
    ...
    #[inline]
    pub fn from_reader(r: &mut impl Reader) -> IoResult<Option<Filter>> {
...
impl<A: Allocator + Clone> Filter<A> {
    ...
    #[inline]
    pub fn from_reader_in(r: &mut impl Reader, alloc: A) -> IoResult<Option<Filter<A>>> {
        if !r.read_bool()? {
            return Ok(None);
        }
...
impl Display for FilterError {
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            FilterError::OsError(c) => Display::fmt(&io::IoError::from_raw_os_error(*c), f),
        }
    }
}
...
impl From<FilterError> for io::Error {
    ...
    fn from(v: FilterError) -> io::Error {
        match v {
            ...
            FilterError::OsError(c) => io::IoError::from_raw_os_error(c),
            FilterError::FindError(m) => io::IoError::new(ErrorKind::Other, m),
        }
    }
}
