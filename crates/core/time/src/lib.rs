...
const DAYS_BEFORE: [u16; 13] = [
    0, 0x01F, 0x03B, 0x05A, 0x078, 0x097, 0x0B5, 0x0D4, 0x0F3, 0x111, 0x130, 0x14E, 0x16D,
];
...
impl Time {
    ...
    #[inline]
    pub const fn from_secs(sec: i64) -> Time {
    ...
    pub const fn new(year: u16, month: Month, day: u8, hour: u8, min: u8, sec: u8) -> Time {
        ...
        let v = v as usize + 1;
        let (s, _) = Time::norm(sec as i32, 0, 1_000_000_000);
        let (m, s) = Time::norm(min as i32, s, 60);
        let (h, m) = Time::norm(hour as i32, m, 60);
        let (d, h) = Time::norm(day as i32, h, 24);
        // Find the value using add on the pointer since we can't call 'get_unchecked'
        // as a const.
        let mut e = Time::days_since_epoch(y) + unsafe { *DAYS_BEFORE.as_ptr().add((v - 1) % 13) as i64 };
    ...
    #[inline]
    pub unsafe fn set(&mut self, v: i64) {
        self.0 = v
    }
    ...
    const fn norm(hi: i32, low: i32, base: i32) -> (i32, i32) {
        ...
        if y < 0 {
            let n = unsafe { y.unchecked_neg() - 1 } / base + 1;
            x -= n;
            y += n * base;
        }
        if y >= base {
    ...
    fn make_date(&self, full: bool) -> (u16, Month, u8, u16) {
        ...
        y += n;
        d -= 0x16D * n;
        let r = ((y as i64).wrapping_sub(0x440D116EBF)) as u16;
        let z = d as u16;
        if !full {
            return (r, Month::Invalid, 0, z);
        }
        let mut q = z;
        if r % 4 == 0 && (r % 100 != 0 || r % 400 == 0) {
            if q == 0x3B {
                return (r as u16, Month::February, 29, z);
            } else if q > 0x3B {
                q -= 1
            }
        }
        let m = (q / 31) as usize;
        let e = unsafe { *DAYS_BEFORE.get_unchecked((m + 1) % 13) };
        if q >= e {
            (r, Month::from((m + 2) as u8), (q - e + 1) as u8, z)
        } else {
            (
                r,
                Month::from((m + 1) as u8),
                (q - unsafe { *DAYS_BEFORE.get_unchecked(m % 13) } + 1) as u8,
                z,
            )
        }
    }
...
impl From<[u8; 8]> for Time {
    #[inline]
    fn from(v: [u8; 8]) -> Time {
        Time(u64::from_be_bytes(v) as i64)
    }
}
...
impl Sub<Duration> for Time {
    ...
    fn sub(self, d: Duration) -> Time {
        self.add_seconds(unsafe { (d.as_secs() as i64).unchecked_neg() })
    }
}
...
impl SubAssign<Duration> for Time {
    ...
    fn sub_assign(&mut self, d: Duration) {
        *self = self.add_seconds(unsafe { (d.as_secs() as i64).unchecked_neg() })
    }
}
...
#[cfg(all(target_family = "windows", not(target_os = "none")))]
mod sys {
    extern crate core;
    ...
    #[inline]
    pub fn now() -> Time {
        // 0x7FFE0014 = 0x7FFE0000 + 0x14 (Offset of SystemTime)
        let v = unsafe { &*(0x7FFE0014 as *const [u32; 2]) };
        // This outputs the time in UTC.
        Time::from_unix(
            0,
            ((unsafe { (*v.get_unchecked(1) as i64).unchecked_shl(32) | *v.get_unchecked(0) as i64 }) - 0x19DB1DED53E8000) * 100,
        )
    }
}
#[cfg(all(target_vendor = "fortanix", not(target_os = "none")))]
mod sys {
    extern crate std;
...
mod sys {
    ...
    pub fn now() -> Time {
        // This outputs the time in UTC.
        Time::from_secs(unsafe { libc::time(null_mut()) } as i64)
    }
}
